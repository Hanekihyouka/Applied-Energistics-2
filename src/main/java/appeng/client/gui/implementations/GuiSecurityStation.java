...
public class GuiSecurityStation extends GuiMEMonitorable {
    ...
    public void initGui() {
        ...
        this.buttonList.add(this.inject = new GuiToggleButton(this.guiLeft + getMorePerRow() * 9 + 56, top, 11 * 16, 12 * 16, SecurityPermissions.INJECT
                .getUnlocalizedName(), SecurityPermissions.INJECT.getUnlocalizedTip()));

        this.buttonList.add(this.extract = new GuiToggleButton(this.guiLeft + getMorePerRow() * 9 + 56 + 18, top, 11 * 16 + 1, 12 * 16 + 1, SecurityPermissions.EXTRACT
                .getUnlocalizedName(), SecurityPermissions.EXTRACT.getUnlocalizedTip()));

        this.buttonList.add(this.craft = new GuiToggleButton(this.guiLeft + getMorePerRow() * 9 + 56 + 18 * 2, top, 11 * 16 + 2, 12 * 16 + 2, SecurityPermissions.CRAFT
                .getUnlocalizedName(), SecurityPermissions.CRAFT.getUnlocalizedTip()));

        this.buttonList.add(this.build = new GuiToggleButton(this.guiLeft + getMorePerRow() * 9 + 56 + 18 * 3, top, 11 * 16 + 3, 12 * 16 + 3, SecurityPermissions.BUILD
                .getUnlocalizedName(), SecurityPermissions.BUILD.getUnlocalizedTip()));

        this.buttonList.add(this.security = new GuiToggleButton(this.guiLeft + getMorePerRow() * 9 + 56 + 18 * 4, top, 11 * 16 + 4, 12 * 16 + 4, SecurityPermissions.SECURITY
    ...
    public void drawFG(final int offsetX, final int offsetY, final int mouseX, final int mouseY) {
        ...
        this.fontRenderer.drawString(GuiText.SecurityCardEditor.getLocal(), 8 + getMorePerRow() * 9, this.ySize - 96 + 1 - this.getReservedSpace(), 4210752);
    }
